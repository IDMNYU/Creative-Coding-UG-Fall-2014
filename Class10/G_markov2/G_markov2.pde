...
void setup()
{
  ...
  wave2 = new Oscil( 440., 0.3, Waves.TRIANGLE ); // this sets up the oscillator
  wave1.patch( out ); // this plugs into the speakers
  wave2.patch( out ); // this plugs into the speakers
  
  mc1 = new lukeMarkov(scale);
  mc2 = new lukeMarkov(scale);
}
...
{
  frameRate(4);
  int pitch1 = mc1.current;
  wave1.setFrequency(mtof(pitch1));
  mc1.tick();
  int pitch2 = mc2.current;
